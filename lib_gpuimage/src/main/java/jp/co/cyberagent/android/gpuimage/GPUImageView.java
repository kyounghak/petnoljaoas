...
public class GPUImageView extends FrameLayout {
     ...
     * the default Picture folder on the phone below the given file object. <br>
    ...
    public void saveToPictures(
        ...
        new SaveTask(file, quality, listener).execute();
    }
    ...
    public void saveToPictures(
            ...
            int quality,
            OnPictureSavedListener listener) {
        new SaveTask(folderName, fileName, quality, listener).execute();
    }
    ...
    public void saveToPictures(
            ...
            int quality,
            OnPictureSavedListener listener) {
        new SaveTask(folderName, fileName, width, height, quality, listener).execute();
    }
    ...
    private class SaveTask extends AsyncTask<Void, Void, Void> {
        ...
        public SaveTask(File file, int qualtiy, OnPictureSavedListener l) {
            this(file, 0, 0, qualtiy, l);
        }

        public SaveTask(final String folderName, final String fileName, int qualtiy, OnPictureSavedListener l) {
            this(folderName, fileName, 0, 0, qualtiy, l);
        }

        SaveTask(File file, int width, int height, int quality, OnPictureSavedListener l) {
            mFile = file;
            mFolderName = null;
            mFileName = null;
            mWidth = width;
        ...
        SaveTask(String folderName, String fileName, int width, int height, int quality, OnPictureSavedListener l) {
            mFile = null;
        ...
        // 외장 폴더에 저장한다.
        // TODO: URI를 가지고 있다가 이미지 업로드에 성공했을 때 외장 폴더에 저장하는 로직으로 변경한다.
        private void saveImage(final String folderName, final String fileName, final Bitmap image) {
            File path = getContext().getApplicationContext().getCacheDir();
            // File path = Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_PICTURES);
            File file = new File(path, fileName);
            Log.d(TAG, "saveImage:file:" + file.toString());
            saveImage(file, image);
        }

        private void saveImage(final File file, final Bitmap image) {
            Log.d(TAG, "saveImage:file:" + file.toString());
            try {
                boolean result = image.compress(Bitmap.CompressFormat.JPEG, mQuality, new FileOutputStream(file));
                if (result) {
                ...
                /*MediaScannerConnection.scanFile(
                        ...
                        new MediaScannerConnection.OnScanCompletedListener() {
                            final String TAG = "OnScanCompletedListener";
