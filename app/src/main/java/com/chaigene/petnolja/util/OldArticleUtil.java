...
public class OldArticleUtil {
     ...
     */
    private static volatile OldArticleUtil mInstance;

    public static OldArticleUtil getInstance() {
        if (mInstance == null) mInstance = new OldArticleUtil();
        return mInstance;
    }
    ...
    /**
     * 해당 해쉬태그를 포함하고 있는 완성된 형태의 Post 목록을 서버로부터 받아오고 메모리에 캐쉬화한다.
     *
     * @param hashtag
     * @param amount
     * @param maxKey
     * @return
     */
    public static Task<List<Post>> getHashtagPosts(final String hashtag, final int amount, final String maxKey) {
        Log.i(TAG, "getUserPosts:amount:" + amount + "/maxKey:" + maxKey);

        return Tasks.call(Executors.newSingleThreadExecutor(), () -> {
            Query query = DatabaseManager.getArticleHashtagsRef().child(hashtag);
    ...
    public static Task<Integer> like(final String postId) {
        ...
        return DatabaseManager.runTransaction(postsRef, new DatabaseManager.TransactionHandler() {
            ...
            if (newFirPost == null) {
                ...
                OldArticleUtil.getInstance().saveCachedPost(newFirPost);
