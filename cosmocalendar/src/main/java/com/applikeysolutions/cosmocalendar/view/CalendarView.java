...
import com.applikeysolutions.cosmocalendar.listeners.OnDaysSelectionListener;
...
import com.applikeysolutions.cosmocalendar.view.customviews.RangeSelectionView;
...
public class CalendarView extends RelativeLayout implements OnDaySelectedListener,
    ...
    // Range mode
    // private LinearLayout vRangeSelection;
    private RangeSelectionView vRangeSelection;
    ...
    private void handleAttributes(TypedArray typedArray) {
        ...
        String monthTextPattern = typedArray.getString(R.styleable.CalendarView_monthTextPattern);
        ...
        settingsManager.setMonthTextPattern(monthTextPattern);
...
//        flBottomSelectionBar.setLayoutTransition(new LayoutTransition());
    ...
    private void createRangeSelectionLayout() {
        // vRangeSelection = (LinearLayout) ((LayoutInflater) getContext().getSystemService(Context.LAYOUT_INFLATER_SERVICE)).inflate(R.layout.view_selection_bar_range, null);
        vRangeSelection.setLayoutParams(new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.WRAP_CONTENT));
        vRangeSelection.setVisibility(GONE);
        flBottomSelectionBar.addView(vRangeSelection);
    }
    ...
    public Month getInitialMonth() {
        Log.i(TAG, "getInitialMonth");
        return monthAdapter.getData().get(SettingsManager.DEFAULT_MONTH_COUNT / 2);
    }
    ...
    // TODO: ??? 이걸 오버라이드 한다고 하더라도 시작일인지 종료일인지 어째 앎?
    // => day 내에 selectionState를 가져와서 알 수 있다.
    @Override
    public void onDaySelected() {
        Log.i(TAG, "onDaySelected");
    ...
    private void displaySelectedDaysRange() {
        Log.i(TAG, "displaySelectedDaysRange");
        if (selectionManager instanceof RangeSelectionManager) {
            Pair<Day, Day> days = ((RangeSelectionManager) selectionManager).getDays();
            if (days != null) {
                vRangeSelection.setVisibility(VISIBLE);
                TextView tvStartRangeTitle = vRangeSelection.findViewById(R.id.tv_range_start_date);
                ...
                TextView tvEndRangeTitle = vRangeSelection.findViewById(R.id.tv_range_end_date);
                ...
                CircleAnimationTextView catvStart = vRangeSelection.findViewById(R.id.catv_start);
                ...
                CircleAnimationTextView catvEnd = vRangeSelection.findViewById(R.id.catv_end);
                ...
                CircleAnimationTextView catvMiddle = vRangeSelection.findViewById(R.id.catv_middle);
    ...
    private void setSelectionBarVisibility() {
        ...
        vRangeSelection.setVisibility(needToShowSelectedDaysRange() ? View.VISIBLE : View.GONE);
    }
    ...
    @Override
    public String getMonthTextPattern() {
        return settingsManager.getMonthTextPattern();
    }
    ...
    @Override
    public void setMonthTextPattern(String monthTextPattern) {
        settingsManager.setMonthTextPattern(monthTextPattern);
        update();
    }
